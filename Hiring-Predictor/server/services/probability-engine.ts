...
export class ProbabilityEngine {
  ...
  private static buildActions(p: PillarScores, job: Job): string[] {
    ...
    if (p.skillFit < 0.7 && job.skills?.length) actions.push("Add or upskill missing core skills from the job description");
    if (p.userBehavior < 0.7) actions.push("Improve resume keywords to mirror the role");
    if (p.userBehavior < 0.6) actions.push("Link GitHub/LinkedIn and add projects to boost completeness");
    if (p.marketContext < 0.5) actions.push("Apply earlier to similar roles to beat applicant surges");
