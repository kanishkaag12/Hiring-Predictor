...
export default function JobCard({ job }: JobCardProps) {
  const getProbabilityColor = (prob: number) => {
    if (prob >= 75) return "bg-emerald-500/15 text-emerald-600 dark:text-emerald-400 border-emerald-500/20";
    if (prob >= 50) return "bg-amber-500/15 text-amber-600 dark:text-amber-400 border-amber-500/20";
    return "bg-rose-500/15 text-rose-600 dark:text-rose-400 border-rose-500/20";
  };

  const getSignalIcon = (signal: string) => {
    if (signal === "Surge") return <TrendingUp className="h-3 w-3 text-emerald-500" />;
    if (signal === "Slowdown") return <TrendingDown className="h-3 w-3 text-rose-500" />;
  ...
  return (
    <Card className="group hover:shadow-lg hover:shadow-primary/5 transition-all duration-300 border-border/60 bg-card/50 backdrop-blur-sm overflow-hidden relative">
      <div className={cn("absolute top-0 right-0 w-24 h-24 bg-gradient-to-bl from-primary/5 to-transparent rounded-bl-full -mr-4 -mt-4 transition-all group-hover:scale-110 duration-500")} />
      ...
      <CardHeader className="p-5 pb-2">
        <div className="flex justify-between items-start">
          ...
          <Badge variant="outline" className={cn("ml-auto font-mono", getProbabilityColor(job.probability))}>
